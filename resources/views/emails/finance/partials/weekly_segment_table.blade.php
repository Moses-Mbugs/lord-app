{{--
  Parameters:
  $segments  – array from WeeklySegmentReportService
...
@php
    ...
    // Full number with commas, keeps sign
    $fmtFull  = fn($v) => number_format((int) round(abs((float) $v)));

    // Coloured movement cell — text only, no badge clutter inside a table
    ...
    // Segment palette: left-accent colour + row/sub-row backgrounds
    $palette = [
...
<table width="100%" cellpadding="0" cellspacing="0"
  ...
  <thead>
    <tr bgcolor="#1B344F">
      ...
      <th style="padding:11px 16px 11px 12px;text-align:left;font-size:10px;font-weight:700;color:#94A3B8;text-transform:uppercase;letter-spacing:0.8px;border-bottom:1px solid #253D54;width:33%;">
        Segment</th>
      <th style="padding:11px 14px;text-align:right;font-size:10px;font-weight:700;color:#94A3B8;text-transform:uppercase;letter-spacing:0.8px;border-bottom:1px solid #253D54;white-space:nowrap;width:16%;">
      ...
      <th style="padding:11px 14px;text-align:right;font-size:10px;font-weight:700;color:#94A3B8;text-transform:uppercase;letter-spacing:0.8px;border-bottom:1px solid #253D54;white-space:nowrap;width:16%;">
        YTD<br><span style="font-size:8.5px;font-weight:500;color:#475569;letter-spacing:0;text-transform:none;">from {{ $fmtShort($ytdStart) }}</span></th>
      <th style="padding:11px 16px 11px 14px;text-align:right;font-size:10px;font-weight:700;color:#94A3B8;text-transform:uppercase;letter-spacing:0.8px;border-bottom:1px solid #253D54;white-space:nowrap;width:19%;">
        Deposits<br><span style="font-size:8.5px;font-weight:500;color:#475569;letter-spacing:0;text-transform:none;">as at {{ $fmtShort($weekEnd) }}</span></th>
    </tr>
  </thead>
  ...
  <tbody>
    @forelse ($segments as $seg)
      @php
        ...
        $visibleSubs = $code === 'OT' ? [] : array_values(array_filter($seg['sub_segments'] ?? [], fn($s) => ($s['name'] ?? '') !== 'Unmapped'));
      ...
      <tr style="background:{{ $p['rowBg'] }};">
        ...
        <td style="padding:11px 14px;text-align:right;{{ $segBorderTop }}">{!! $mvCell($seg['ytd_mv'] ?? 0) !!}</td>
      ...
      @foreach ($seg['sub_segments'] ?? [] as $si => $sub)
        @php
          if (($sub['name'] ?? '') === 'Unmapped' || $code === 'OT') continue;
          $isLastSub = $loop->last;
    ...
    @empty
      <tr>
        <td colspan="6" style="padding:28px;text-align:center;color:#94A3B8;font-size:12px;">
